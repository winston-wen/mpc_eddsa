...
impl PublicKey {
    ...
    pub fn verify_strict(
        ...
        if true {
            ...
            println!("inside verify_strict(), r: {}", r);

            let sig_s = signature.s.to_bytes();
            let sig_s = bs58::encode(sig_s).into_string();
            println!("inside verify_strict(), sig_s: {}", sig_s);
            let sig_r = signature_R.compress().to_bytes(); // identical bytes, either or not compress
            let sig_r = bs58::encode(sig_r).into_string();
            println!("inside verify_strict(), sig_r: {}", sig_r);
        }
